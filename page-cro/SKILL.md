...
1. **CRO audit report** (page URL, current CR, benchmark comparison, improvement potential)
2. **Above-the-fold audit** (headline clarity, value prop, CTA visibility, copy analysis)
3. **Critical issues list** (issues, fixes, impact, effort level)
4. **High-impact opportunities** (changes with expected conversion lift)
5. **Quick wins list** (under-1-hour changes with impact estimate)
6. **Detailed section-by-section recommendations** (copy rewrites, design suggestions, social proof, form optimization)
7. **Test-ready hypotheses** (mapped to ab-test-setup skill for execution)

This skill does NOT produce: A/B test execution, traffic splitting, or result monitoring.
...
### Test Hypothesis Handoff

For each recommended change, produce a test-ready hypothesis for the `ab-test-setup` skill:

| Change | Hypothesis | Metric | Expected Impact |
|--------|-----------|--------|-----------------|
| [Recommended CRO change] | "If we [change], then [metric] will [improve/increase] by [X%] because [rationale]" | [Primary metric] | [Estimated lift] |

To execute: run `ab-test-setup` with these hypotheses as input. The ab-test-setup skill will handle sample size calculation, test duration, and statistical rigor.

## Quality Checks

- [ ] Did you audit every section of the page (above the fold, headline, subhead, proof, CTA, form, objection handling)?
- [ ] Are recommendations specific enough to implement without interpretation? (Exact copy rewrites, not "improve the headline")
- [ ] Did you produce test-ready hypotheses for each recommended change (suitable for `ab-test-setup`)?
- [ ] Did you prioritize by impact/effort ratio — not just list everything that could be better?
- [ ] Did you compare against industry benchmarks, not just internal numbers?
- [ ] If a brand-voice file was provided, do copy rewrites match it?
