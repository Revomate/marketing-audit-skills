...
1. **Traffic overview report** (sessions, users, page views, engagement metrics by date range)
2. **User acquisition breakdown** by channel (source, medium, campaign, conversions)
3. **Top pages report** (ranked by views, users, engagement rate, duration)
4. **Conversion tracking report** (events, user counts, event value by type or channel)
5. **Audience segment breakdown** (by device, geography, landing page, source)
6. **Period comparison report** (current vs. previous period with trend indicators)

This skill does NOT produce: Analytics tool integration, automated scheduled reports, custom dashboards, or data storage.
...
- [ ] Did you verify the GA4 property ID is correct before pulling data?
- [ ] Are date ranges explicitly stated in every report (no ambiguous "recent" data)?
- [ ] Did you provide actionable interpretation — not just raw numbers?
- [ ] For traffic reports: did you segment by channel, not just show totals?
- [ ] For conversion reports: did you calculate conversion rates, not just counts?
- [ ] Did you flag any data quality issues (missing tracking, low sample size, bot traffic)?
- [ ] Does the report include recommended next actions based on the data?
