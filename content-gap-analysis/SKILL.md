...
1. **Keyword gaps table** (keyword, volume, difficulty, competitor, content type needed, priority)
2. **Topic cluster gaps** (missing clusters, under-covered clusters with subtopic lists)
3. **Content format gaps** (missing formats competitors use, with action and priority)
4. **Buyer journey gap analysis** (missing content by stage: awareness through advocacy)
5. **AI/GEO visibility gap analysis** (keywords in AI overviews, citation status, required fixes)
6. **Prioritized content plan** (ranked with content piece, type, keyword, volume, priority score, gap type)
7. **Quick wins list** (low-effort, high-impact content to create immediately)

This skill does NOT produce: Content creation, publishing, or performance tracking.
...
## Web-Only Mode (No SemRush API)

When the SemRush API is not available, use this manual process to identify content gaps using only web search and site crawling.

### Step 3 Alternative: Manual Keyword Gap Analysis

**3a. Harvest competitor keywords from sitemaps and pages:**
1. Fetch each competitor's sitemap (`/sitemap.xml`) — extract all blog/resource URLs
2. For each URL, use WebFetch to read the page. Extract:
   - Title tag (reveals target keyword)
   - H1 heading
   - URL slug (often contains the keyword)
3. Build a list of competitor target keywords from these signals

**3b. Cross-reference against your own content:**
1. Fetch your sitemap and extract the same data
2. Compare: which topics do competitors target that you don't?
3. For each gap, run a WebSearch query like `"{keyword}" site:{your_domain}` — if zero results, it's confirmed missing

**3c. Validate search demand with free tools:**
- Use WebSearch for `{keyword} search volume` — SEO blogs often publish volume estimates
- Check Google autocomplete: type the keyword and note suggestions (signals demand)
- Check "People Also Ask" in Google results for the keyword (signals related demand)
- Use Google Trends (via WebFetch) to compare relative interest across gap keywords

**3d. Estimate keyword difficulty manually:**
- Search the keyword in Google
- Note the top 5 domains: Are they all DR 80+ authority sites? → High difficulty
- Are there forums, small blogs, or thin content ranking? → Lower difficulty, good opportunity
- Is there a featured snippet? → Opportunity to win it with better-structured content

### Step 3 Alternative Output

Format the manual analysis the same way as the API-powered version:

| Gap Keyword | Source | Competitor Ranking | Demand Signal | Difficulty Estimate | Content Type Needed |
|-------------|--------|-------------------|---------------|--------------------|--------------------|
| {keyword} | {competitor sitemap} | {competitor} has a page | {autocomplete/PAA/trends} | {Low/Med/High based on SERP scan} | {type} |

**Accuracy note:** Manual analysis provides directional guidance, not exact volume/difficulty numbers. Prioritize gaps where multiple signals align (competitor covers it + Google autocomplete suggests it + PAA questions exist).
